...
public class NetworkManagerAbove23 implements NetworkManagerInterface {
    ...
    private ConnectivityManager.NetworkCallback mNetworkCallback = new ConnectivityManager.NetworkCallback() { // from class: org.linphone.core.tools.network.NetworkManagerAbove23.1
        ...
        public void onAvailable(final Network network) {
            NetworkManagerAbove23.this.mHelper.getHandler().post(new Runnable() { // from class: org.linphone.core.tools.network.NetworkManagerAbove23.1.1
                ...
                public void run() {
                    NetworkInfo networkInfo = NetworkManagerAbove23.this.mConnectivityManager.getNetworkInfo(network);
                    if (networkInfo == null) {
    ...
    public NetworkManagerAbove23(AndroidPlatformHelper androidPlatformHelper, ConnectivityManager connectivityManager, boolean z) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (arrayList.contains(hostAddress) || arrayList2.contains(hostAddress)) {
                            ...
                            if (activeNetwork != null && network2.equals(activeNetwork)) {
                                ...
                                objArr[0] = "[Platform Helper] [Network Manager 23] Found DNS host " + hostAddress + " from active network " + typeName;
                                Log.i(objArr);
                                arrayList2.add(hostAddress);
                                network = activeNetwork;
                            } else if (zHasLinkPropertiesDefaultRoute) {
                                network = activeNetwork;
                                Log.i("[Platform Helper] [Network Manager 23] Found DNS host " + hostAddress + " from network " + typeName + " with default route");
                                arrayList.add(0, hostAddress);
                            } else {
                                network = activeNetwork;
                                Log.i("[Platform Helper] [Network Manager 23] Found DNS host " + hostAddress + " from network " + typeName);
