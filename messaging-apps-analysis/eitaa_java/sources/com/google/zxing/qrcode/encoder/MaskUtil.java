...
final class MaskUtil {
    ...
    private static boolean isWhiteHorizontal(byte[] rowArray, int from, int to) {
        int iMin = Math.min(to, rowArray.length);
        for (int iMax = Math.max(from, 0); iMax < iMin; iMax++) {
            if (rowArray[iMax] == 1) {
                return false;
            }
        }
    ...
    private static boolean isWhiteVertical(byte[][] array, int col, int from, int to) {
        int iMin = Math.min(to, array.length);
        for (int iMax = Math.max(from, 0); iMax < iMin; iMax++) {
            if (array[iMax][col] == 1) {
                return false;
            }
    ...
    static int applyMaskPenaltyRule4(ByteMatrix matrix) {
        ...
        int height2 = matrix.getHeight() * matrix.getWidth();
    ...
    */
    ...
    private static int applyMaskPenaltyRule1Internal(ByteMatrix matrix, boolean isHorizontal) {
        int height = isHorizontal ? matrix.getHeight() : matrix.getWidth();
        int width = isHorizontal ? matrix.getWidth() : matrix.getHeight();
        byte[][] array = matrix.getArray();
        int i = 0;
        for (int i2 = 0; i2 < height; i2++) {
            byte b = -1;
            ...
            for (int i4 = 0; i4 < width; i4++) {
                byte b2 = isHorizontal ? array[i2][i4] : array[i4][i2];
                if (b2 == b) {
                    i3++;
                } else {
                    if (i3 >= 5) {
