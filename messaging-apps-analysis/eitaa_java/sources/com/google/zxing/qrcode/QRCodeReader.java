...
public class QRCodeReader {
    ...
    public final Result decode(BinaryBitmap image, Map<DecodeHintType, ?> hints) throws NotFoundException, ChecksumException, FormatException {
        ...
        if (hints != null && hints.containsKey(DecodeHintType.PURE_BARCODE)) {
            decoderResultDecode = this.decoder.decode(extractPureBits(image.getBlackMatrix()), hints);
            points = NO_POINTS;
        } else {
            DetectorResult detectorResultDetect = new Detector(image.getBlackMatrix()).detect(hints);
            DecoderResult decoderResultDecode2 = this.decoder.decode(detectorResultDetect.getBits(), hints);
    ...
    private static BitMatrix extractPureBits(BitMatrix image) throws NotFoundException {
        ...
        float fModuleSize = moduleSize(topLeftOnBit, image);
        ...
        BitMatrix bitMatrix = new BitMatrix(iRound, iRound2, 1);
        for (int i11 = 0; i11 < iRound2; i11++) {
            int i12 = ((int) (i11 * fModuleSize)) + i7;
            for (int i13 = 0; i13 < iRound; i13++) {
                if (image.get(((int) (i13 * fModuleSize)) + i8, i12)) {
