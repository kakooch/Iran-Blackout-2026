...
public class PatternCell extends BackupImageView implements DownloadController.FileDownloadProgressListener {
    ...
    public PatternCell(Context context, int maxSize, PatternCellDelegate patternCellDelegate) {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            setOutlineProvider(new ViewOutlineProvider() { // from class: ir.eitaa.ui.Cells.PatternCell.1
    ...
    public void setPattern(TLRPC$TL_wallPaper wallPaper) {
        ...
        if (wallPaper != null) {
            setImage(ImageLocation.getForDocument(FileLoader.getClosestPhotoSizeWithSize(wallPaper.document.thumbs, 100), wallPaper.document), "100_100", null, null, "jpg", 0, 1, wallPaper);
        } else {
    ...
    private void updateButtonState(Object image, boolean ifSame, boolean animated) {
        File httpFilePath;
        String name;
        boolean z = image instanceof TLRPC$TL_wallPaper;
        if (z || (image instanceof MediaController.SearchImage)) {
            if (z) {
                TLRPC$TL_wallPaper tLRPC$TL_wallPaper = (TLRPC$TL_wallPaper) image;
                name = FileLoader.getAttachFileName(tLRPC$TL_wallPaper.document);
                if (TextUtils.isEmpty(name)) {
                    return;
                } else {
                    httpFilePath = FileLoader.getPathToAttach(tLRPC$TL_wallPaper.document, true);
                }
            } else {
                MediaController.SearchImage searchImage = (MediaController.SearchImage) image;
    ...
    public void onFailedDownload(String fileName, boolean canceled) {
        ...
        if ((tLRPC$TL_wallPaper == null && selectedPattern == null) || !(selectedPattern == null || tLRPC$TL_wallPaper == null || tLRPC$TL_wallPaper.id != selectedPattern.id)) {
            if (canceled) {
                ...
                updateButtonState(tLRPC$TL_wallPaper, true, canceled);
            }
        }
    }
