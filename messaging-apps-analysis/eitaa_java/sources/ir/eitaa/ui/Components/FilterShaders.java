...
public class FilterShaders {
    ...
    public static String vertexShaderForOptimizedBlurOfRadius(int blurRadius, float sigma) {
        int i;
        float[] fArr = new float[(blurRadius * 2) + 1];
        float f = 0.0f;
        int i2 = 0;
        while (true) {
            i = blurRadius + 1;
            ...
            double d = sigma;
            fArr[i2] = (float) ((1.0d / Math.sqrt(Math.pow(d, 2.0d) * 6.283185307179586d)) * Math.exp((-Math.pow(i2, 2.0d)) / (Math.pow(d, 2.0d) * 2.0d)));
            if (i2 == 0) {
                f += fArr[i2];
            } else {
                double d2 = f;
        ...
        int iMin = Math.min((blurRadius / 2) + (blurRadius % 2), 7);
    ...
    private static class BlurProgram {
        ...
        public BlurProgram(float radius, float sigma, boolean calc) {
            ...
            this.fragmentShaderCode = FilterShaders.fragmentShaderForOptimizedBlurOfRadius(i, sigma);
            this.vertexShaderCode = FilterShaders.vertexShaderForOptimizedBlurOfRadius(i, sigma);
        }
    ...
    private static class ToneCurve {
        ...
        private float[] getPreparedSplineCurve(ArrayList<PointF> points) {
            ...
            ArrayList<PointF> arrayListSplineCurve = splineCurve(points);
        ...
        private ArrayList<PointF> splineCurve(ArrayList<PointF> points) {
            double[] dArrSecondDerivative = secondDerivative(points);
            int length = dArrSecondDerivative.length;
            if (length < 1) {
                return null;
            }
            ArrayList<PointF> arrayList = new ArrayList<>(length + 1);
            int i = 0;
            while (i < length - 1) {
                PointF pointF = points.get(i);
                int i2 = i + 1;
                ...
                while (true) {
                    ...
                    if (i3 < ((int) f)) {
                        ...
                        Double.isNaN(d5);
                        Double.isNaN(d5);
                        float f5 = (float) ((d6 * d4) + (d7 * d3) + (((d5 * d5) / 6.0d) * (((((d4 * d4) * d4) - d4) * dArrSecondDerivative[i]) + ((((d3 * d3) * d3) - d3) * dArrSecondDerivative[i5]))));
                        if (f5 > 255.0f) {
                            f5 = 255.0f;
                        } else if (f5 < 0.0f) {
                            f5 = 0.0f;
        ...
        private void updateToneCurveTexture() {
            ...
            ByteBuffer byteBufferAllocateDirect = ByteBuffer.allocateDirect(Factory.DEVICE_MCH264ENC_NO_PIX_FMT_CONV);
            ...
            GLES20.glTexImage2D(3553, 0, 6408, Factory.DEVICE_HAS_CRAPPY_OPENSLES, 1, 0, 6408, 5121, byteBufferAllocateDirect);
        }
    ...
    public boolean create() {
        ...
        if (iLoadShader6 == 0 || iLoadShader7 == 0) {
            return false;
        }
        int iGlCreateProgram2 = GLES20.glCreateProgram();
        this.sharpenShaderProgram = iGlCreateProgram2;
        GLES20.glAttachShader(iGlCreateProgram2, iLoadShader6);
        GLES20.glAttachShader(this.sharpenShaderProgram, iLoadShader7);
        ...
        int iLoadShader10 = loadShader(35633, "attribute vec4 position;attribute vec2 inputTexCoord;varying vec2 texCoord;void main() {gl_Position = position;texCoord = inputTexCoord;}");
        int iLoadShader11 = loadShader(35632, "varying highp vec2 texCoord;uniform sampler2D sourceImage;uniform sampler2D inputImageTexture2;uniform lowp float excludeSize;uniform lowp vec2 excludePoint;uniform lowp float excludeBlurSize;uniform highp float aspectRatio;void main() {lowp vec4 sharpImageColor = texture2D(sourceImage, texCoord);lowp vec4 blurredImageColor = texture2D(inputImageTexture2, texCoord);highp vec2 texCoordToUse = vec2(texCoord.x, (texCoord.y * aspectRatio + 0.5 - 0.5 * aspectRatio));highp float distanceFromCenter = distance(excludePoint, texCoordToUse);gl_FragColor = mix(sharpImageColor, blurredImageColor, smoothstep(excludeSize - excludeBlurSize, excludeSize, distanceFromCenter));}");
        if (iLoadShader10 == 0 || iLoadShader11 == 0) {
            return false;
        }
        int iGlCreateProgram4 = GLES20.glCreateProgram();
        this.radialBlurShaderProgram = iGlCreateProgram4;
        GLES20.glAttachShader(iGlCreateProgram4, iLoadShader10);
        GLES20.glAttachShader(this.radialBlurShaderProgram, iLoadShader11);
        ...
        while (true) {
            ...
            if (i2 >= (z2 ? 2 : 1)) {
                ...
                if (this.isVideo) {
                    iLoadShader = loadShader(35633, "attribute vec4 position;uniform mat4 videoMatrix;attribute vec4 inputTexCoord;varying vec2 texCoord;void main() {gl_Position = position;texCoord = vec2(videoMatrix * inputTexCoord).xy;}");
                } else {
                    iLoadShader = loadShader(35633, "attribute vec4 position;attribute vec2 inputTexCoord;varying vec2 texCoord;void main() {gl_Position = position;texCoord = inputTexCoord;}");
                }
                Locale locale = Locale.US;
                int iLoadShader14 = loadShader(35632, String.format(locale, "%1$s\nprecision lowp float;varying highp vec2 texCoord;uniform %2$s sourceImage;void main() {vec4 inp = texture2D(sourceImage, texCoord);vec4 image = vec4(inp.rgb * pow(2.0, -1.0), inp.w);vec4 base = vec4(image.g, image.g, image.g, 1.0);vec4 overlay = vec4(image.b, image.b, image.b, 1.0);float ba = 2.0 * overlay.b * base.b + overlay.b * (1.0 - base.a) + base.b * (1.0 - overlay.a);gl_FragColor = vec4(ba,ba,ba,image.a);}", str3, str4));
                if (iLoadShader == 0 || iLoadShader14 == 0) {
                    return false;
                }
                int iGlCreateProgram6 = GLES20.glCreateProgram();
                this.greenAndBlueChannelOverlayProgram = iGlCreateProgram6;
                GLES20.glAttachShader(iGlCreateProgram6, iLoadShader);
                GLES20.glAttachShader(this.greenAndBlueChannelOverlayProgram, iLoadShader14);
                ...
                int iLoadShader15 = loadShader(35632, "precision lowp float;varying highp vec2 texCoord;varying highp vec2 texCoord2;uniform sampler2D sourceImage;uniform sampler2D inputImageTexture2;void main() {vec4 image = texture2D(sourceImage, texCoord);vec4 blurredImage = texture2D(inputImageTexture2, texCoord2);gl_FragColor = vec4((image.rgb - blurredImage.rgb + vec3(0.5,0.5,0.5)), image.a);}");
                int iLoadShader16 = loadShader(35633, "attribute vec4 position;attribute vec2 inputTexCoord;varying vec2 texCoord;varying vec2 texCoord2;void main() {gl_Position = position;texCoord = inputTexCoord;texCoord2 = inputTexCoord;}");
                if (iLoadShader16 == 0 || iLoadShader15 == 0) {
                    return false;
                }
                int iGlCreateProgram7 = GLES20.glCreateProgram();
                this.highPassProgram = iGlCreateProgram7;
                GLES20.glAttachShader(iGlCreateProgram7, iLoadShader16);
                GLES20.glAttachShader(this.highPassProgram, iLoadShader15);
                ...
                if (this.isVideo) {
                    iLoadShader2 = loadShader(35633, "attribute vec4 position;uniform mat4 videoMatrix;attribute vec4 inputTexCoord;varying vec2 texCoord;varying vec2 texCoord2;void main() {gl_Position = position;texCoord = vec2(videoMatrix * inputTexCoord).xy;texCoord2 = inputTexCoord.xy;}");
                } else {
                    iLoadShader2 = loadShader(35633, "attribute vec4 position;attribute vec2 inputTexCoord;varying vec2 texCoord;varying vec2 texCoord2;void main() {gl_Position = position;texCoord = inputTexCoord;texCoord2 = inputTexCoord;}");
                }
                ...
                if (iLoadShader2 == 0 || iLoadShader19 == 0) {
                    return false;
                }
                int iGlCreateProgram9 = GLES20.glCreateProgram();
                this.compositeProgram = iGlCreateProgram9;
                GLES20.glAttachShader(iGlCreateProgram9, iLoadShader2);
                GLES20.glAttachShader(this.compositeProgram, iLoadShader19);
            ...
            if (i2 == 1 && z2) {
                int iLoadShader20 = loadShader(35633, "attribute vec4 position;uniform mat4 videoMatrix;attribute vec4 inputTexCoord;varying vec2 texCoord;void main() {gl_Position = position;texCoord = vec2(videoMatrix * inputTexCoord).xy;}");
                Locale locale2 = Locale.US;
                str2 = str5;
                Object[] objArr = new Object[2];
                objArr[c] = str3;
                objArr[1] = str4;
                iLoadShader3 = loadShader(35632, String.format(locale2, "%1$s\nprecision highp float;varying vec2 texCoord;uniform %2$s sourceImage;vec3 rgb_to_hsv(vec3 c) {vec4 K = vec4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);vec4 p = c.g < c.b ? vec4(c.bg, K.wz) : vec4(c.gb, K.xy);vec4 q = c.r < p.x ? vec4(p.xyw, c.r) : vec4(c.r, p.yzx);float d = q.x - min(q.w, q.y);float e = 1.0e-10;return vec3(abs(q.z + (q.w - q.y) / (6.0 * d + e)), d / (q.x + e), q.x);}void main() {vec4 texel = texture2D(sourceImage, texCoord);gl_FragColor = vec4(rgb_to_hsv(texel.rgb), texel.a);}", objArr));
                i = iLoadShader20;
            } else {
                str2 = str5;
                int iLoadShader21 = loadShader(35633, "attribute vec4 position;attribute vec2 inputTexCoord;varying vec2 texCoord;void main() {gl_Position = position;texCoord = inputTexCoord;}");
                iLoadShader3 = loadShader(35632, String.format(Locale.US, "%1$s\nprecision highp float;varying vec2 texCoord;uniform %2$s sourceImage;vec3 rgb_to_hsv(vec3 c) {vec4 K = vec4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);vec4 p = c.g < c.b ? vec4(c.bg, K.wz) : vec4(c.gb, K.xy);vec4 q = c.r < p.x ? vec4(p.xyw, c.r) : vec4(c.r, p.yzx);float d = q.x - min(q.w, q.y);float e = 1.0e-10;return vec3(abs(q.z + (q.w - q.y) / (6.0 * d + e)), d / (q.x + e), q.x);}void main() {vec4 texel = texture2D(sourceImage, texCoord);gl_FragColor = vec4(rgb_to_hsv(texel.rgb), texel.a);}", "", "sampler2D"));
                i = iLoadShader21;
            }
            if (i == 0 || iLoadShader3 == 0) {
                return false;
            }
            this.rgbToHsvShaderProgram[i2] = GLES20.glCreateProgram();
            GLES20.glAttachShader(this.rgbToHsvShaderProgram[i2], i);
            GLES20.glAttachShader(this.rgbToHsvShaderProgram[i2], iLoadShader3);
    ...
    public void drawEnhancePass() {
        ...
        if (!this.hsvGenerated) {
            ...
            GLES20.glBindTexture(3553, this.enhanceTextures[1]);
            GLES20.glTexImage2D(3553, 0, 6408, Factory.DEVICE_HAS_CRAPPY_OPENSLES, 16, 0, 6408, 5121, this.cdtBuffer);
            if (!this.isVideo) {
                this.hsvBuffer = null;
                this.cdtBuffer = null;
    ...
    private void loadTexture(Bitmap bitmap, int orientation, int w, int h) {
        ...
        if (bitmapCreateBitmap != null && !bitmap.isRecycled()) {
            ...
            if (i > photoSize || this.renderBufferHeight > photoSize || orientation % 360 != 0) {
                float width = 1.0f;
                if (i > photoSize || this.renderBufferHeight > photoSize) {
                ...
                if (i2 == 90 || i2 == 270) {
                    int i3 = this.renderBufferWidth;
                    this.renderBufferWidth = this.renderBufferHeight;
                    this.renderBufferHeight = i3;
                }
                bitmapCreateBitmap = createBitmap(bitmapCreateBitmap, orientation, width);
            }
            GLES20.glBindTexture(3553, this.bitmapTextre[0]);
            GLES20.glTexParameteri(3553, 10241, 9729);
            GLES20.glTexParameteri(3553, 10240, 9729);
        ...
        for (int i4 = 0; i4 < 2; i4++) {
            GLES20.glBindTexture(3553, this.renderTexture[i4]);
            GLES20.glTexParameteri(3553, 10241, 9729);
            GLES20.glTexParameteri(3553, 10240, 9729);
            GLES20.glTexParameteri(3553, 10242, 33071);
            GLES20.glTexParameteri(3553, 10243, 33071);
    ...
    public static FilterShadersDelegate getFilterShadersDelegate(final MediaController.SavedFilterState lastState) {
        return new FilterShadersDelegate() { // from class: ir.eitaa.ui.Components.FilterShaders.1
            @Override // ir.eitaa.ui.Components.FilterShaders.FilterShadersDelegate
            ...
            public float getTintHighlightsIntensityValue() {
                return lastState.tintHighlightsColor == 0 ? 0.0f : 0.5f;
            }

            @Override // ir.eitaa.ui.Components.FilterShaders.FilterShadersDelegate
            public float getTintShadowsIntensityValue() {
                return lastState.tintShadowsColor == 0 ? 0.0f : 0.5f;
            }

            @Override // ir.eitaa.ui.Components.FilterShaders.FilterShadersDelegate
            public float getSaturationValue() {
                float f = lastState.saturationValue / 100.0f;
                if (f > 0.0f) {
                    f *= 1.05f;
                }
                return f + 1.0f;
