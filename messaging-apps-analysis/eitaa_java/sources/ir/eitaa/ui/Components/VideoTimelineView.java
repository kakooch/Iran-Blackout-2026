...
public class VideoTimelineView extends View {
    ...
    public void reloadFrames(int frameNum) {
        ...
        AsyncTask<Integer, Integer, Bitmap> asyncTask = new AsyncTask<Integer, Integer, Bitmap>() { // from class: ir.eitaa.ui.Components.VideoTimelineView.1
    ...
    private void drawCorners(Canvas canvas, int height, int width, int left, int top) {
        ...
        int i2 = this.roundCornersSize >> 1;
        canvas.save();
        float f = left;
        float f2 = top;
        canvas.drawBitmap(this.roundCornerBitmap, f, f2, (Paint) null);
        float f3 = (height + top) - i2;
        canvas.rotate(-90.0f, left + i2, f3);
        canvas.drawBitmap(this.roundCornerBitmap, f, r9 - this.roundCornersSize, (Paint) null);
        canvas.restore();
        canvas.save();
        float f4 = (left + width) - i2;
        canvas.rotate(180.0f, f4, f3);
        Bitmap bitmap = this.roundCornerBitmap;
        int i3 = this.roundCornersSize;
        canvas.drawBitmap(bitmap, r11 - i3, r9 - i3, (Paint) null);
        canvas.restore();
        canvas.save();
        canvas.rotate(90.0f, f4, top + i2);
    ...
    public static class TimeHintView extends View {
        ...
        public void setTime(int timeInSeconds) {
            ...
            if (j != this.lastTime) {
                this.lastTime = j;
                String shortDuration = AndroidUtilities.formatShortDuration(timeInSeconds);
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            super.onMeasure(widthMeasureSpec, View.MeasureSpec.makeMeasureSpec(this.tooltipLayout.getHeight() + AndroidUtilities.dp(4.0f) + this.tooltipBackgroundArrow.getIntrinsicHeight(), 1073741824));
        }
