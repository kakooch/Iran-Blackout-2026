...
public class CallSwipeView extends View {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (ev.getAction() == 0) {
            ...
            if (ev.getAction() == 2) {
                this.viewToDrag.setTranslationX(Math.max(this.dragFromRight ? -(getWidth() - getDraggedViewWidth()) : 0.0f, Math.min(ev.getX() - this.dragStartX, this.dragFromRight ? 0.0f : getWidth() - getDraggedViewWidth())));
                invalidate();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float fAbs = Math.abs(this.viewToDrag.getTranslationX());
        for (int i = 0; i < 3; i++) {
            float f = 16.0f;
            this.arrowsPaint.setAlpha(Math.round(this.arrowAlphas[i] * (fAbs > ((float) AndroidUtilities.dp((float) (i * 16))) ? 1.0f - Math.min(1.0f, Math.max(0.0f, (fAbs - (AndroidUtilities.dp(16.0f) * i)) / AndroidUtilities.dp(16.0f))) : 1.0f)));
    ...
    private class ArrowAnimWrapper {
        ...
        public void setArrowAlpha(int value) {
            this.this$0.arrowAlphas[this.index] = value;
        }
    }
}
