...
public class Input {
    ...
    public void process(MotionEvent event, float scale) {
        ...
        float x = event.getX();
        float height = this.renderView.getHeight() - event.getY();
        ...
        if (point.getDistanceTo(this.lastLocation) < AndroidUtilities.dp(5.0f) / scale) {
            return;
        }
        if (!this.hasMoved) {
    ...
    private Point smoothPoint(Point midPoint1, Point midPoint2, Point prev1, float t) {
        float f = 1.0f - t;
        double dPow = Math.pow(f, 2.0d);
        double d = f * 2.0f * t;
        double d2 = t * t;
        double d3 = midPoint1.x * dPow;
        double d4 = prev1.x;
        Double.isNaN(d);
        double d5 = d3 + (d4 * d);
        double d6 = midPoint2.x;
        Double.isNaN(d2);
        double d7 = midPoint1.y * dPow;
        double d8 = prev1.y;
        Double.isNaN(d);
        double d9 = midPoint2.y;
        Double.isNaN(d2);
    ...
    private void paintPath(final Path path) {
        ...
        this.renderView.getPainting().paintStroke(path, this.clearBuffer, new Runnable() { // from class: ir.eitaa.ui.Components.Paint.-$$Lambda$Input$R9U9ct0eNpBvCneq-Eq6md4dyBA
            @Override // java.lang.Runnable
            public final void run() {
                this.f$0.lambda$paintPath$1$Input(path);
            }
        });
    ...
    /* renamed from: lambda$paintPath$0, reason: merged with bridge method [inline-methods] */
    public /* synthetic */ void lambda$paintPath$0$Input(Path path) {
        this.lastRemainder = path.remainder;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: lambda$paintPath$1, reason: merged with bridge method [inline-methods] */
    public /* synthetic */ void lambda$paintPath$1$Input(final Path path) {
        AndroidUtilities.runOnUIThread(new Runnable() { // from class: ir.eitaa.ui.Components.Paint.-$$Lambda$Input$-RkV2XtcuFvOGYHrIMw5JqAx4Z8
            @Override // java.lang.Runnable
            public final void run() {
                this.f$0.lambda$paintPath$0$Input(path);
            }
        });
    }
}
