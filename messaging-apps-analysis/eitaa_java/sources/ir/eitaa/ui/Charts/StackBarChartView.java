...
import ir.eitaa.ui.Charts.view_data.StackBarViewData;
...
public class StackBarChartView extends BaseChartView<StackBarChartData, StackBarViewData> {
            ...
            if (this.selectedIndex != iMax || !z) {
                ...
                while (i5 < this.lines.size()) {
                    ...
                    if (lineViewData.enabled || lineViewData.alpha != f13) {
                        ...
                        float f16 = ((f2 / f12) + (((StackBarChartData) this.chartData).xPercentage[iMax] * (f8 - f2))) - f10;
                        float measuredHeight = (iArr[iMax] / this.currentMaxHeight) * ((getMeasuredHeight() - this.chartBottom) - BaseChartView.SIGNATURE_TEXT_HEIGHT) * lineViewData.alpha;
    ...
    protected void selectXOnChart(int x, int y) {
        ...
        float f3 = (x + f2) / (f - (((StackBarChartData) t).xPercentage.length < 2 ? 1.0f : ((StackBarChartData) t).xPercentage[1] * f));
        ...
        if (BaseChartView.ANIMATE_PICKER_SIZES) {
            ...
            if (i > 0) {
                ...
                if (f != this.animatedToPickerMaxHeight) {
                    ...
                    ValueAnimator valueAnimatorCreateAnimator = createAnimator(this.pickerMaxHeight, this.animatedToPickerMaxHeight, new ValueAnimator.AnimatorUpdateListener() { // from class: ir.eitaa.ui.Charts.StackBarChartView.1
