...
public class EncryptionKeyEmojifier {
    ...
    private static int bytesToInt(byte[] arr, int offset) {
        return (arr[offset + 3] & 255) | ((arr[offset] & 127) << 24) | ((arr[offset + 1] & 255) << 16) | ((arr[offset + 2] & 255) << 8);
    }

    private static long bytesToLong(byte[] arr, int offset) {
        return (arr[offset + 7] & 255) | ((arr[offset] & 127) << 56) | ((arr[offset + 1] & 255) << 48) | ((arr[offset + 2] & 255) << 40) | ((arr[offset + 3] & 255) << 32) | ((arr[offset + 4] & 255) << 24) | ((arr[offset + 5] & 255) << 16) | ((arr[offset + 6] & 255) << 8);
    }
    ...
    public static String[] emojify(byte[] sha256) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            strArr[i] = strArr2[bytesToInt(sha256, offsets[i]) % strArr2.length];
        }
        return strArr;
    }
    ...
    public static String[] emojifyForCall(byte[] sha256) {
        ...
        for (int i = 0; i < 4; i++) {
            strArr[i] = emojis[(int) (bytesToLong(sha256, i * 8) % r3.length)];
        }
        return strArr;
    }
}
