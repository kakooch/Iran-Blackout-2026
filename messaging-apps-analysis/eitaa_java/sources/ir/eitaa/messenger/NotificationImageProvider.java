...
public class NotificationImageProvider extends ContentProvider implements NotificationCenter.NotificationCenterDelegate {
    public static final String AUTHORITY = "ir.eitaa.messenger.notification_image_provider";
    ...
    @Override // android.content.ContentProvider
    public ParcelFileDescriptor openFile(Uri uri, String mode) throws NumberFormatException, IOException {
        if (!"r".equals(mode)) {
        ...
        if (matcher.match(uri) == 1) {
            ...
            if (!file.exists()) {
                Long l = this.fileStartTimes.get(str);
                long jLongValue = l != null ? l.longValue() : System.currentTimeMillis();
                if (l == null) {
                    this.fileStartTimes.put(str, Long.valueOf(jLongValue));
                }
                while (!file.exists()) {
                    if (System.currentTimeMillis() - jLongValue >= 3000) {
                        if (BuildVars.LOGS_ENABLED) {
                            FileLog.w("Waiting for " + str + " to download timed out");
                        }
