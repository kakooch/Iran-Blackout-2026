...
public class MusicPlayerReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) throws InterruptedException, IOException {
        ...
        action.hashCode();
        switch (action) {
            case "ir.eitaa.android.musicplayer.next":
                MediaController.getInstance().playNextMessage();
                break;
            case "ir.eitaa.android.musicplayer.play":
                MediaController.getInstance().playMessage(MediaController.getInstance().getPlayingMessageObject());
                break;
            case "android.media.AUDIO_BECOMING_NOISY":
            case "ir.eitaa.android.musicplayer.pause":
                MediaController.getInstance().lambda$startAudioAgain$7$MediaController(MediaController.getInstance().getPlayingMessageObject());
                break;
            case "ir.eitaa.android.musicplayer.close":
                MediaController.getInstance().cleanupPlayer(true, true);
                break;
            case "ir.eitaa.android.musicplayer.previous":
                MediaController.getInstance().playPreviousMessage();
                break;
        }
    }
