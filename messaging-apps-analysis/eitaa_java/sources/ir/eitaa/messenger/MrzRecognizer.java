...
public class MrzRecognizer {
    ...
    public static Result recognize(Bitmap bitmap, boolean tryDriverLicenseFirst) {
        ...
        if (tryDriverLicenseFirst && (resultRecognizeBarcode2 = recognizeBarcode(bitmap)) != null) {
        ...
        if (tryDriverLicenseFirst || (resultRecognizeBarcode = recognizeBarcode(bitmap)) == null) {
            return null;
        }
    ...
    */
    private static ir.eitaa.messenger.MrzRecognizer.Result recognizeMRZ(android.graphics.Bitmap r25) throws java.lang.NumberFormatException {
        /*
            Method dump skipped, instructions count: 1921
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: ir.eitaa.messenger.MrzRecognizer.recognizeMRZ(android.graphics.Bitmap):ir.eitaa.messenger.MrzRecognizer$Result");
    }

    public static Result recognize(byte[] yuvData, int width, int height, int rotation) {
        Bitmap bitmapCreateBitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        setYuvBitmapPixels(bitmapCreateBitmap, yuvData);
        Matrix matrix = new Matrix();
        matrix.setRotate(rotation);
        int iMin = Math.min(width, height);
        int iRound = Math.round(iMin * 0.704f);
        boolean z = rotation == 90 || rotation == 270;
        return recognize(Bitmap.createBitmap(bitmapCreateBitmap, z ? (width / 2) - (iRound / 2) : 0, z ? 0 : (height / 2) - (iRound / 2), z ? iRound : iMin, z ? iMin : iRound, matrix, false), false);
    }
    ...
    private static void parseBirthDate(String birthDate, Result result) throws NumberFormatException {
        try {
            int i = Integer.parseInt(birthDate.substring(0, 2));
            result.birthYear = i;
    ...
    private static String cyrillicToLatin(String s) {
        ...
        while (i < 33) {
            int i2 = i + 1;
            s = s.replace("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ".substring(i, i2), strArr[i]);
            i = i2;
        }
        return s;
    }
