...
public class TLRPC$TL_phone_groupCall extends TLObject {
    ...
    public static TLRPC$TL_phone_groupCall TLdeserialize(AbstractSerializedData stream, int constructor2, boolean exception) {
        if (constructor != constructor2) {
            if (exception) {
                throw new RuntimeException(String.format("can't parse magic %x in TL_phone_groupCall", Integer.valueOf(constructor2)));
            }
            return null;
        }
        TLRPC$TL_phone_groupCall tLRPC$TL_phone_groupCall = new TLRPC$TL_phone_groupCall();
        tLRPC$TL_phone_groupCall.readParams(stream, exception);
