...
public class TLRPC$TL_phone_joinAsPeers extends TLObject {
    ...
    public static TLRPC$TL_phone_joinAsPeers TLdeserialize(AbstractSerializedData stream, int constructor2, boolean exception) {
        if (constructor != constructor2) {
            if (exception) {
                throw new RuntimeException(String.format("can't parse magic %x in TL_phone_joinAsPeers", Integer.valueOf(constructor2)));
            }
            return null;
        }
        TLRPC$TL_phone_joinAsPeers tLRPC$TL_phone_joinAsPeers = new TLRPC$TL_phone_joinAsPeers();
        tLRPC$TL_phone_joinAsPeers.readParams(stream, exception);
    ...
    public void readParams(AbstractSerializedData stream, boolean exception) {
        ...
        for (int i = 0; i < int322; i++) {
            ...
            this.peers.add(tLRPC$PeerTLdeserialize);
        }
