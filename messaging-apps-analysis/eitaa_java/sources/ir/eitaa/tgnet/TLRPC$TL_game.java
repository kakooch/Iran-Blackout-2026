...
public class TLRPC$TL_game extends TLObject {
    ...
    public static TLRPC$TL_game TLdeserialize(AbstractSerializedData stream, int constructor2, boolean exception) {
        if (constructor != constructor2) {
            if (exception) {
                throw new RuntimeException(String.format("can't parse magic %x in TL_game", Integer.valueOf(constructor2)));
            }
            return null;
        }
        TLRPC$TL_game tLRPC$TL_game = new TLRPC$TL_game();
        tLRPC$TL_game.readParams(stream, exception);
