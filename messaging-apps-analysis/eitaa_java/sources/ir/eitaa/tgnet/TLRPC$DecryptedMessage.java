...
public abstract class TLRPC$DecryptedMessage extends TLObject {
    ...
    public static TLRPC$DecryptedMessage TLdeserialize(AbstractSerializedData stream, int constructor, boolean exception) {
        ...
        if (tLRPC$TL_decryptedMessage == null && exception) {
            throw new RuntimeException(String.format("can't parse magic %x in DecryptedMessage", Integer.valueOf(constructor)));
        }
        if (tLRPC$TL_decryptedMessage != null) {
            tLRPC$TL_decryptedMessage.readParams(stream, exception);
        }
