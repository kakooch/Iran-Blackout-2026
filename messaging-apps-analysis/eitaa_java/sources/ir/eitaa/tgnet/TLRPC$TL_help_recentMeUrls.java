...
public class TLRPC$TL_help_recentMeUrls extends TLObject {
    ...
    public static TLRPC$TL_help_recentMeUrls TLdeserialize(AbstractSerializedData stream, int constructor2, boolean exception) {
        if (constructor != constructor2) {
            if (exception) {
                throw new RuntimeException(String.format("can't parse magic %x in TL_help_recentMeUrls", Integer.valueOf(constructor2)));
            }
            return null;
        }
        TLRPC$TL_help_recentMeUrls tLRPC$TL_help_recentMeUrls = new TLRPC$TL_help_recentMeUrls();
        tLRPC$TL_help_recentMeUrls.readParams(stream, exception);
    ...
    public void readParams(AbstractSerializedData stream, boolean exception) {
        ...
        for (int i = 0; i < int322; i++) {
            TLRPC$RecentMeUrl tLRPC$RecentMeUrlTLdeserialize = TLRPC$RecentMeUrl.TLdeserialize(stream, stream.readInt32(exception), exception);
            if (tLRPC$RecentMeUrlTLdeserialize == null) {
                return;
            }
            this.urls.add(tLRPC$RecentMeUrlTLdeserialize);
        }
    ...
    public void serializeToStream(AbstractSerializedData stream) {
        ...
        for (int i = 0; i < size; i++) {
            this.urls.get(i).serializeToStream(stream);
        }
        stream.writeInt32(481674261);
