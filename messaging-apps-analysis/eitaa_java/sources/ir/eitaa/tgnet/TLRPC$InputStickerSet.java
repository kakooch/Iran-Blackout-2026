...
public abstract class TLRPC$InputStickerSet extends TLObject {
    ...
    public static TLRPC$InputStickerSet TLdeserialize(AbstractSerializedData stream, int constructor, boolean exception) {
        ...
        if (tLRPC$TL_inputStickerSetShortName == null && exception) {
            throw new RuntimeException(String.format("can't parse magic %x in InputStickerSet", Integer.valueOf(constructor)));
        }
        if (tLRPC$TL_inputStickerSetShortName != null) {
            tLRPC$TL_inputStickerSetShortName.readParams(stream, exception);
        }
