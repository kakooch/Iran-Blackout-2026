...
public class TLRPC$TL_messages_inactiveChats extends TLObject {
    ...
    public static TLRPC$TL_messages_inactiveChats TLdeserialize(AbstractSerializedData stream, int constructor2, boolean exception) {
        if (constructor != constructor2) {
            if (exception) {
                throw new RuntimeException(String.format("can't parse magic %x in TL_messages_inactiveChats", Integer.valueOf(constructor2)));
            }
            return null;
        }
        TLRPC$TL_messages_inactiveChats tLRPC$TL_messages_inactiveChats = new TLRPC$TL_messages_inactiveChats();
        tLRPC$TL_messages_inactiveChats.readParams(stream, exception);
    ...
    public void serializeToStream(AbstractSerializedData stream) {
        ...
        for (int i = 0; i < size; i++) {
            stream.writeInt32(this.dates.get(i).intValue());
        }
        stream.writeInt32(481674261);
