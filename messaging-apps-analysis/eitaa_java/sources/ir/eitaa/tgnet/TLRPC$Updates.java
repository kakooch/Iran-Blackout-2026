...
public abstract class TLRPC$Updates extends TLObject {
    ...
    public static TLRPC$Updates TLdeserialize(AbstractSerializedData stream, int constructor, boolean exception) {
        ...
        if (tLRPC$TL_updateShortSentMessage == null && exception) {
            throw new RuntimeException(String.format("can't parse magic %x in Updates", Integer.valueOf(constructor)));
        }
        if (tLRPC$TL_updateShortSentMessage != null) {
            tLRPC$TL_updateShortSentMessage.readParams(stream, exception);
        }
