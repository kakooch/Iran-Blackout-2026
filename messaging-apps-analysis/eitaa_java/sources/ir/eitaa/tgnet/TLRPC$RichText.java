...
public abstract class TLRPC$RichText extends TLObject {
    ...
    public static TLRPC$RichText TLdeserialize(AbstractSerializedData stream, int constructor, boolean exception) {
        ...
        if (tLRPC$TL_textStrike == null && exception) {
            throw new RuntimeException(String.format("can't parse magic %x in RichText", Integer.valueOf(constructor)));
        }
        if (tLRPC$TL_textStrike != null) {
            tLRPC$TL_textStrike.readParams(stream, exception);
        }
