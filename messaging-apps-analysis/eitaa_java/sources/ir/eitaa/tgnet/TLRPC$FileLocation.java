...
public abstract class TLRPC$FileLocation extends TLObject {
    ...
    public static TLRPC$FileLocation TLdeserialize(AbstractSerializedData stream, int constructor, boolean exception) {
        ...
        if (tLRPC$TL_fileLocationToBeDeprecated == null && exception) {
            throw new RuntimeException(String.format("can't parse magic %x in FileLocation", Integer.valueOf(constructor)));
        }
        if (tLRPC$TL_fileLocationToBeDeprecated != null) {
            tLRPC$TL_fileLocationToBeDeprecated.readParams(stream, exception);
        }
