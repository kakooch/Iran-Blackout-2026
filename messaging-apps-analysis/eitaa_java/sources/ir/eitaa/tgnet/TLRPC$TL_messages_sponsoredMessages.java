...
public class TLRPC$TL_messages_sponsoredMessages extends TLObject {
    ...
    public static TLRPC$TL_messages_sponsoredMessages TLdeserialize(AbstractSerializedData stream, int constructor2, boolean exception) {
        if (constructor != constructor2) {
            if (exception) {
                throw new RuntimeException(String.format("can't parse magic %x in TL_messages_sponsoredMessages", Integer.valueOf(constructor2)));
            }
            return null;
        }
        TLRPC$TL_messages_sponsoredMessages tLRPC$TL_messages_sponsoredMessages = new TLRPC$TL_messages_sponsoredMessages();
        tLRPC$TL_messages_sponsoredMessages.readParams(stream, exception);
    ...
    public void readParams(AbstractSerializedData stream, boolean exception) {
        ...
        for (int i = 0; i < int322; i++) {
            TLRPC$TL_sponsoredMessage tLRPC$TL_sponsoredMessageTLdeserialize = TLRPC$TL_sponsoredMessage.TLdeserialize(stream, stream.readInt32(exception), exception);
            if (tLRPC$TL_sponsoredMessageTLdeserialize == null) {
                return;
            }
            this.messages.add(tLRPC$TL_sponsoredMessageTLdeserialize);
        }
