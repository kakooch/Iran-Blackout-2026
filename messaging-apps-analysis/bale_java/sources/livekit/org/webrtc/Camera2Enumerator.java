            ...
            try {
                ...
                try {
                    ...
                    for (Size size : supportedSizes) {
                        ...
                        int iRound = outputMinFrameDuration == 0 ? iMax : ((int) Math.round(NANO_SECONDS_PER_SECOND / outputMinFrameDuration)) * AppMetricaDefaultValues.DEFAULT_MAX_REPORTS_IN_DATABASE_COUNT;
                        arrayList.add(new CameraEnumerationAndroid.CaptureFormat(size.width, size.height, 0, iRound));
                        Logging.d(TAG, "Format: " + size.width + "x" + size.height + Separators.AT + iRound);
                    }
