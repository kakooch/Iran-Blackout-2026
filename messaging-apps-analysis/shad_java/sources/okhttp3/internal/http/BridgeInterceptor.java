    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (requestBodyBody != null) {
            ...
            if (mediaTypeContentType != null) {
                builderNewBuilder.header(com.facebook.stetho.server.http.HttpHeaders.CONTENT_TYPE, mediaTypeContentType.toString());
            }
            long jContentLength = requestBodyBody.contentLength();
            if (jContentLength != -1) {
                builderNewBuilder.header(com.facebook.stetho.server.http.HttpHeaders.CONTENT_LENGTH, Long.toString(jContentLength));
        ...
        if (z && "gzip".equalsIgnoreCase(responseProceed.header("Content-Encoding")) && HttpHeaders.hasBody(responseProceed)) {
            ...
            builderRequest.headers(responseProceed.headers().newBuilder().removeAll("Content-Encoding").removeAll(com.facebook.stetho.server.http.HttpHeaders.CONTENT_LENGTH).build());
            builderRequest.body(new RealResponseBody(responseProceed.header(com.facebook.stetho.server.http.HttpHeaders.CONTENT_TYPE), -1L, Okio.buffer(gzipSource)));
        }
