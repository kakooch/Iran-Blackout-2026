...
public class SharedPreferencesDumperPlugin implements DumperPlugin {
    ...
    public void dump(DumperContext dumperContext) throws DumpUsageException {
        ...
        String strRemove = argsAsList.isEmpty() ? BuildConfig.FLAVOR : argsAsList.remove(0);
    ...
    private void doPrint(PrintStream printStream, List<String> list) {
        printRecursive(printStream, this.mAppContext.getApplicationInfo().dataDir + "/shared_prefs", BuildConfig.FLAVOR, list.isEmpty() ? BuildConfig.FLAVOR : list.get(0), list.size() > 1 ? list.get(1) : BuildConfig.FLAVOR);
    }
