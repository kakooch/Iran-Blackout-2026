...
public final class DashMediaSource extends BaseMediaSource {
    ...
    public DashMediaSource(Uri uri, DataSource.Factory factory, ParsingLoadable.Parser<? extends DashManifest> parser, DashChunkSource.Factory factory2, int i, long j, Handler handler, MediaSourceEventListener mediaSourceEventListener) {
        this(new MediaItem.Builder().setUri(uri).setMimeType("application/dash+xml").build(), null, factory, parser, factory2, new DefaultCompositeSequenceableLoaderFactory(), DrmSessionManager.CC.getDummyDrmSessionManager(), new DefaultLoadErrorHandlingPolicy(i), j == -1 ? 30000L : j, j != -1);
    ...
    */
    ...
    private void processManifest(boolean z) {
        ...
        refreshSourceInfo(new DashTimeline(dashManifest3.availabilityStartTimeMs, jUsToMs, this.elapsedRealtimeOffsetMs, this.firstPeriodId, j, periodDurationUs2, j2, dashManifest3, this.mediaItem));
        if (this.sideloadedManifest) {
            return;
        }
        this.handler.removeCallbacks(this.simulateManifestRefreshRunnable);
    ...
    private static final class DashTimeline extends Timeline {
        ...
        public Timeline.Window getWindow(int i, Timeline.Window window, long j) {
            ...
            DashManifest dashManifest = this.manifest;
            return window.set(obj, mediaItem, dashManifest, this.presentationStartTimeMs, this.windowStartTimeMs, this.elapsedRealtimeEpochOffsetMs, true, isMovingLiveWindow(dashManifest), this.manifest.dynamic, adjustedWindowDefaultStartPositionUs, this.windowDurationUs, 0, getPeriodCount() - 1, this.offsetInFirstPeriodUs);
        }
