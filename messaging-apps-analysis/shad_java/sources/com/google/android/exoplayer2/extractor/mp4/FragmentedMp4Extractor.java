...
public class FragmentedMp4Extractor implements Extractor {
    ...
    private static void parseTraf(Atom.ContainerAtom containerAtom, SparseArray<TrackBundle> sparseArray, int i, byte[] bArr) throws ParserException {
        TrackBundle tfhd = parseTfhd(((Atom.LeafAtom) Assertions.checkNotNull(containerAtom.getLeafAtomOfType(1952868452))).data, sparseArray);
        if (tfhd == null) {
            return;
        }
    ...
    private static void parseSaiz(TrackEncryptionBox trackEncryptionBox, ParsableByteArray parsableByteArray, TrackFragment trackFragment) throws ParserException {
        ...
        if (unsignedIntToInt > trackFragment.sampleCount) {
            throw new ParserException("Saiz sample count " + unsignedIntToInt + " is greater than fragment sample count" + trackFragment.sampleCount);
        }
        if (unsignedByte == 0) {
    ...
    */
    ...
    private static void parseSenc(ParsableByteArray parsableByteArray, int i, TrackFragment trackFragment) throws ParserException {
        ...
        if (unsignedIntToInt != trackFragment.sampleCount) {
            throw new ParserException("Senc sample count " + unsignedIntToInt + " is different from fragment sample count" + trackFragment.sampleCount);
        }
