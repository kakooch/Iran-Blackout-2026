...
final class VideoTagPayloadReader extends TagPayloadReader {
    ...
    protected boolean parsePayload(ParsableByteArray parsableByteArray, long j) throws ParserException {
        ...
        if (unsignedByte == 0 && !this.hasOutputFormat) {
            ...
            this.output.format(new Format.Builder().setSampleMimeType(MediaController.VIDEO_MIME_TYPE).setWidth(avcConfig.width).setHeight(avcConfig.height).setPixelWidthHeightRatio(avcConfig.pixelWidthAspectRatio).setInitializationData(avcConfig.initializationData).build());
