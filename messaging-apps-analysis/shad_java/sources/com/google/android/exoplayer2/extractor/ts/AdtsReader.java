...
public final class AdtsReader implements ElementaryStreamReader {
    ...
    public void createTracks(ExtractorOutput extractorOutput, TsPayloadReader.TrackIdGenerator trackIdGenerator) {
        ...
        if (this.exposeId3) {
            trackIdGenerator.generateNewId();
            TrackOutput trackOutputTrack2 = extractorOutput.track(trackIdGenerator.getTrackId(), 4);
    ...
    private void parseAdtsHeader() throws ParserException {
        ...
        if (!this.hasOutputFormat) {
            ...
            Format formatBuild = new Format.Builder().setId(this.formatId).setSampleMimeType(MediaController.AUIDO_MIME_TYPE).setCodecs(audioSpecificConfig.codecs).setChannelCount(audioSpecificConfig.channelCount).setSampleRate(audioSpecificConfig.sampleRateHz).setInitializationData(Collections.singletonList(bArrBuildAudioSpecificConfig)).setLanguage(this.language).build();
