    ...
    private void parseStreamMuxConfig(ParsableBitArray parsableBitArray) throws ParserException {
        ...
        if (bits2 == 0) {
            ...
            if (bits == 0) {
                ...
                Format formatBuild = new Format.Builder().setId(this.formatId).setSampleMimeType(MediaController.AUIDO_MIME_TYPE).setCodecs(this.codecs).setChannelCount(this.channelCount).setSampleRate(this.sampleRateHz).setInitializationData(Collections.singletonList(bArr)).setLanguage(this.language).build();
