...
public class Processor implements ExecutionListener, ForegroundProcessor {
    ...
    public boolean startWork(String id, WorkerParameters.RuntimeExtras runtimeExtras) {
        synchronized (this.mLock) {
            ...
            Logger.get().debug(TAG, String.format("%s: processing %s", Processor.class.getSimpleName(), id), new Throwable[0]);
            return true;
        }
    }
    ...
    public void onExecuted(final String workSpecId, boolean needsReschedule) {
        synchronized (this.mLock) {
            ...
            Logger.get().debug(TAG, String.format("%s %s executed; reschedule = %s", Processor.class.getSimpleName(), workSpecId, Boolean.valueOf(needsReschedule)), new Throwable[0]);
