...
public class NativeByteBuffer extends AbstractSerializedData {
    private static final ThreadLocal<NativeByteBuffer> addressWrapper = new ThreadLocal<NativeByteBuffer>() { // from class: org.rbmain.tgnet.NativeByteBuffer.1
    ...
    public static NativeByteBuffer wrap(long j) {
        ...
        if (j != 0) {
            ...
            nativeByteBuffer.buffer = byteBufferNative_getJavaByteBuffer;
            byteBufferNative_getJavaByteBuffer.limit(native_limit(j));
            int iNative_position = native_position(j);
            if (iNative_position <= nativeByteBuffer.buffer.limit()) {
                nativeByteBuffer.buffer.position(iNative_position);
            }
    ...
    @Override // org.rbmain.tgnet.AbstractSerializedData
    ...
    public void writeString(String str) {
        try {
            writeByteArray(str.getBytes(Utf8Charset.NAME));
        } catch (Exception e) {
            if (BuildVars.LOGS_ENABLED) {
