...
public class SmsReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        try {
            ...
            if (matcher.find()) {
                final String strReplace = matcher.group(0).replace("-", io.github.inflationx.calligraphy3.BuildConfig.FLAVOR);
                if (strReplace.length() >= 3) {
                    if (string != null) {
                        sharedPreferences.edit().putString("sms_hash_code", string + "|" + strReplace).apply();
                    }
                    AndroidUtilities.runOnUIThread(new Runnable() { // from class: org.rbmain.messenger.SmsReceiver$$ExternalSyntheticLambda0
