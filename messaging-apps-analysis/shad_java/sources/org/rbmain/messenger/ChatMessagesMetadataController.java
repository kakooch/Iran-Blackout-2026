...
import org.rbmain.tgnet.TLRPC$TL_messages_getExtendedMedia;
import org.rbmain.tgnet.TLRPC$TL_messages_getMessagesReactions;
...
public class ChatMessagesMetadataController {
    ...
    public void loadReactionsForMessages(long j, ArrayList<MessageObject> arrayList) {
        ...
        tLRPC$TL_messages_getMessagesReactions.peer = this.chatActivity.getMessagesController().getInputPeer((int) j);
        for (int i = 0; i < arrayList.size(); i++) {
            tLRPC$TL_messages_getMessagesReactions.id.add(Long.valueOf(arrayList.get(i).getId()));
        }
        this.reactionsRequests.add(Integer.valueOf(this.chatActivity.getConnectionsManager().sendRequest(tLRPC$TL_messages_getMessagesReactions, new RequestDelegate() { // from class: org.rbmain.messenger.ChatMessagesMetadataController$$ExternalSyntheticLambda0
            @Override // org.rbmain.tgnet.RequestDelegate
            public final void run(TLObject tLObject, TLRPC$TL_error tLRPC$TL_error) {
                this.f$0.lambda$loadReactionsForMessages$0(tLObject, tLRPC$TL_error);
            }
        })));
    ...
    public void loadExtendedMediaForMessages(long j, ArrayList<MessageObject> arrayList) {
        ...
        tLRPC$TL_messages_getExtendedMedia.peer = this.chatActivity.getMessagesController().getInputPeer((int) j);
        for (int i = 0; i < arrayList.size(); i++) {
            tLRPC$TL_messages_getExtendedMedia.id.add(Long.valueOf(arrayList.get(i).getId()));
        }
        this.extendedMediaRequests.add(Integer.valueOf(this.chatActivity.getConnectionsManager().sendRequest(tLRPC$TL_messages_getExtendedMedia, new RequestDelegate() { // from class: org.rbmain.messenger.ChatMessagesMetadataController$$ExternalSyntheticLambda1
            @Override // org.rbmain.tgnet.RequestDelegate
            public final void run(TLObject tLObject, TLRPC$TL_error tLRPC$TL_error) {
                this.f$0.lambda$loadExtendedMediaForMessages$1(tLObject, tLRPC$TL_error);
            }
        })));
