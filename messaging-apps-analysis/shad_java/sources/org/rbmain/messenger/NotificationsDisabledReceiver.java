    ...
    public void onReceive(Context context, Intent intent) {
        if ("android.app.action.NOTIFICATION_CHANNEL_BLOCK_STATE_CHANGED".equals(intent.getAction())) {
            ...
            if (zStartsWith) {
                ...
                if (!stringExtra.equals(notificationsSettings.getString("org.rbmain.key" + jLongValue, null))) {
                    return;
                }
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.d("apply channel " + stringExtra + " state");
                ...
                editorEdit2.putInt(NotificationsSettingsFacade.PROPERTY_NOTIFY + jLongValue, booleanExtra ? 2 : 0);
                editorEdit2.putInt(NotificationsSettingsFacade.PROPERTY_NOTIFY + IdStorage.getInstance().getDialogId(jLongValue), booleanExtra ? 2 : 0);
                if (!booleanExtra) {
                    editorEdit2.remove(NotificationsSettingsFacade.PROPERTY_NOTIFY_UNTIL + jLongValue);
                }
                editorEdit2.apply();
