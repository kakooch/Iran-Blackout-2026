...
import org.rbmain.ui.Components.AvatarsDrawable;
...
public class ReactionsLayoutInBubble {
    ...
    private static final Comparator<TLRPC$User> usersComparator = new Comparator() { // from class: org.rbmain.ui.Components.Reactions.ReactionsLayoutInBubble$$ExternalSyntheticLambda1
    ...
    public static /* synthetic */ int lambda$static$0(TLRPC$User tLRPC$User, TLRPC$User tLRPC$User2) {
        return tLRPC$User.id - tLRPC$User2.id;
    }
                    ...
                    if (!z && messageObject.messageOwner.reactions.recent_reactions != null) {
                        if (messageObject.getDialogId() > 0 && !UserObject.isReplyUser(messageObject.getDialogId())) {
                            ...
                            for (int i4 = 0; i4 < messageObject.messageOwner.reactions.recent_reactions.size(); i4++) {
                                ...
                                if (VisibleReaction.fromTLReaction(tLRPC$MessagePeerReaction.reaction).equals(VisibleReaction.fromTLReaction(tLRPC$ReactionCount.reaction)) && MessagesController.getInstance(this.currentAccount).getUser(Integer.valueOf(MessageObject.getPeerId(tLRPC$MessagePeerReaction.peer_id))) != null && (user = MessagesController.getInstance(this.currentAccount).getUser(Integer.valueOf(MessageObject.getPeerId(tLRPC$MessagePeerReaction.peer_id)))) != null) {
                                    if (arrayList3 == null) {
    ...
    public void measure(int i, int i2) {
        ...
        for (int i4 = 0; i4 < this.reactionButtons.size(); i4++) {
            ReactionButton reactionButton = this.reactionButtons.get(i4);
        ...
        if (i2 == 5 && !this.reactionButtons.isEmpty()) {
            int i5 = this.reactionButtons.get(0).y;
            int i6 = 0;
            for (int i7 = 0; i7 < this.reactionButtons.size(); i7++) {
                if (this.reactionButtons.get(i7).y != i5) {
                    int i8 = i7 - 1;
                    int i9 = i - (this.reactionButtons.get(i8).x + this.reactionButtons.get(i8).width);
                    while (i6 < i7) {
                        this.reactionButtons.get(i6).x += i9;
                        i6++;
                    }
                    i6 = i7;
                }
            ...
            int i10 = i - (this.reactionButtons.get(size).x + this.reactionButtons.get(size).width);
            while (i6 <= size) {
                this.reactionButtons.get(i6).x += i10;
                i6++;
            }
        }
        this.lastLineX = iDp;
        ...
        this.height = iDp2 + (this.reactionButtons.size() != 0 ? AndroidUtilities.dp(26.0f) : 0);
    ...
    public ReactionButton getReactionButton(VisibleReaction visibleReaction) {
        ...
        if (string == null) {
            string = Long.toString(visibleReaction.documentId);
        }
        if (this.isSmall) {
            ReactionButton reactionButton = this.lastDrawingReactionButtons.get(string + "_");
            if (reactionButton != null) {
                return reactionButton;
            }
        }
        return this.lastDrawingReactionButtons.get(string);
    }
    ...
    public class ReactionButton {
            ...
            if (this.reaction != null) {
                ...
                if (visibleReaction.emojicon != null) {
                    ...
                    if (tLRPC$TL_availableReaction != null) {
                        DocumentObject.getSvgThumb(tLRPC$TL_availableReaction.static_icon, Theme.key_windowBackgroundGray, 1.0f);
                        this.imageReceiver.setImage(ImageLocation.getForDocument(tLRPC$TL_availableReaction.center_icon), "40_40_lastreactframe", Emoji.getEmojiDrawable(this.visibleReaction.emojicon), "webp", tLRPC$TL_availableReaction, 1);
                    }
        ...
        public void draw(Canvas canvas, float f, float f2, boolean z) {
            ...
            if (imageReceiver != null) {
                ...
                int i2 = (int) ((this.height - iDp2) / 2.0f);
                this.drawingImageRect.set(iDp, i2, iDp + iDp2, iDp2 + i2);
                imageReceiver.setImageCoords(this.drawingImageRect);
                drawImage(canvas, f2);
            }
            CounterView.CounterDrawable counterDrawable = this.counterDrawable;
            ...
            while (true) {
                ...
                if (x <= this.reactionButtons.get(i).x || x >= this.reactionButtons.get(i).x + this.reactionButtons.get(i).width || y <= this.reactionButtons.get(i).y || y >= this.reactionButtons.get(i).y + this.reactionButtons.get(i).height) {
                    ...
                    if (messageObject2.messageOwner.reactions.can_see_list || messageObject2.getDialogId() >= 0) {
                        Runnable runnable2 = new Runnable() { // from class: org.rbmain.ui.Components.Reactions.ReactionsLayoutInBubble$$ExternalSyntheticLambda0
                            @Override // java.lang.Runnable
                            public final void run() {
                                this.f$0.lambda$chekTouchEvent$1(reactionButton);
                            }
                        };
    ...
    public static class VisibleReaction {
        ...
        public static VisibleReaction fromTLReaction(TLRPC$Reaction tLRPC$Reaction) {
            ...
            if (tLRPC$Reaction instanceof TLRPC$TL_reactionEmoji) {
                ...
                str.hashCode();
                visibleReaction.reactionId = tLRPC$Reaction.reactionId;
        ...
        public static VisibleReaction fromEmojicon(TLRPC$TL_availableReaction tLRPC$TL_availableReaction) {
            ...
            visibleReaction.emojicon = str;
            str.hashCode();
            visibleReaction.reactionId = tLRPC$TL_availableReaction.reaction_id;
            return visibleReaction;
        }
        ...
        public boolean equals(Object obj) {
            ...
            return this.documentId == visibleReaction.documentId && CallProxy$$ExternalSyntheticBackport0.m(this.emojicon, visibleReaction.emojicon);
        }
