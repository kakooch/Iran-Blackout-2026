...
public class SpoilerEffectBitmapFactory {
    ...
    private SpoilerEffectBitmapFactory() {
        new Matrix();
        int iDp = AndroidUtilities.dp(SharedConfig.getDevicePerformanceClass() == 2 ? 200.0f : 150.0f);
        Point point = AndroidUtilities.displaySize;
        int iMin = (int) Math.min(Math.min(point.x, point.y) * 0.5f, iDp);
        this.size = iMin;
        if (iMin < AndroidUtilities.dp(100.0f)) {
        ...
        if (this.shaderBitmap == null) {
            ...
            for (int i4 = 0; i4 < 10; i4++) {
                for (int i5 = 0; i5 < 10; i5++) {
                    ...
                    spoilerEffect.particlePoints = (float[][]) Array.newInstance((Class<?>) float.class, SpoilerEffect.ALPHAS.length, iDp * 2);
                    spoilerEffect.setMaxParticlesCount(iDp);
    ...
    public void checkUpdate() {
        ...
        this.dispatchQueue.postRunnable(new Runnable() { // from class: org.rbmain.ui.Components.spoilers.SpoilerEffectBitmapFactory$$ExternalSyntheticLambda1
    ...
    public /* synthetic */ void lambda$checkUpdate$1(final Bitmap bitmap) {
        ...
        AndroidUtilities.runOnUIThread(new Runnable() { // from class: org.rbmain.ui.Components.spoilers.SpoilerEffectBitmapFactory$$ExternalSyntheticLambda0
            @Override // java.lang.Runnable
            public final void run() {
