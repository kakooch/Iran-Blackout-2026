...
public class SeekBarView extends FrameLayout {
    ...
    public interface SeekBarViewDelegate {
        ...
        /* renamed from: org.rbmain.ui.Components.SeekBarView$SeekBarViewDelegate$-CC, reason: invalid class name */
    ...
    public SeekBarView(Context context, boolean z, Theme.ResourcesProvider resourcesProvider) {
        ...
        FloatSeekBarAccessibilityDelegate floatSeekBarAccessibilityDelegate = new FloatSeekBarAccessibilityDelegate(z) { // from class: org.rbmain.ui.Components.SeekBarView.1
    ...
    boolean onTouch(MotionEvent motionEvent) {
        ...
        if (motionEvent.getAction() == 1 || motionEvent.getAction() == 3) {
            ...
            if (this.pressed) {
                ...
                this.pressed = false;
                AndroidUtilities.runOnUIThread(new Runnable() { // from class: org.rbmain.ui.Components.SeekBarView$$ExternalSyntheticLambda0
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float fDp2 = measuredHeight2 + (AndroidUtilities.dp(this.lineWidthDp) / 2.0f);
        this.rect.set(f, fDp, getMeasuredWidth() - (this.selectorWidth / 2), fDp2);
        ...
        if (this.twoSided) {
            ...
            this.rect.set(f, fDp, (this.selectorWidth / 2) + i2, fDp2);
        ...
        if (f2 != f3) {
            if (f2 < f3) {
                ...
                float fDp4 = f2 - (AndroidUtilities.dp(1.0f) * (jElapsedRealtime / 60.0f));
        ...
        if (f4 < 1.0f) {
            float f5 = f4 + (jElapsedRealtime / 225.0f);
            this.transitionProgress = f5;
            if (f5 >= 1.0f) {
                this.transitionProgress = 1.0f;
                z = z2;
            }
        ...
        if (f6 < 1.0f) {
            float interpolation = 1.0f - Easings.easeInQuad.getInterpolation(Math.min(1.0f, f6 * 3.0f));
    ...
    public void updateTimestamps(MessageObject messageObject, Long l) {
        ...
        try {
            ...
            Collections.sort(this.timestamps, new Comparator() { // from class: org.rbmain.ui.Components.SeekBarView$$ExternalSyntheticLambda1
    ...
    private void drawProgressBar(Canvas canvas, RectF rectF, Paint paint) {
        ...
        float fDp2 = AndroidUtilities.dp(this.timestampsAppearing * 1.0f) / 2.0f;
        if (tmpPath == null) {
            tmpPath = new Path();
        }
        tmpPath.reset();
        float fDp3 = AndroidUtilities.dp(4.0f) / (measuredWidth - f4);
        int i = 0;
        while (true) {
            size = -1;
            if (i >= this.timestamps.size()) {
                i = -1;
                break;
            } else if (((Float) this.timestamps.get(i).first).floatValue() >= fDp3) {
                break;
            } else {
                i++;
            }
        ...
        while (true) {
            ...
            if (1.0f - ((Float) this.timestamps.get(size2).first).floatValue() >= fDp3) {
                size = size2 + 1;
                break;
            }
            size2--;
        ...
        int i3 = i;
        while (i3 <= size) {
            float fFloatValue = i3 == i ? 0.0f : ((Float) this.timestamps.get(i3 - 1).first).floatValue();
            float fFloatValue2 = i3 == size ? 1.0f : ((Float) this.timestamps.get(i3).first).floatValue();
            while (i3 != size && i3 != 0 && i3 < this.timestamps.size() - i2 && ((Float) this.timestamps.get(i3).first).floatValue() - fFloatValue <= fDp3) {
                i3++;
                fFloatValue2 = ((Float) this.timestamps.get(i3).first).floatValue();
            }
            RectF rectF2 = AndroidUtilities.rectTmp;
            rectF2.left = AndroidUtilities.lerp(f4, measuredWidth, fFloatValue) + (i3 > 0 ? fDp2 : 0.0f);
            float fLerp = AndroidUtilities.lerp(f4, measuredWidth, fFloatValue2) - (i3 < size ? fDp2 : 0.0f);
            rectF2.right = fLerp;
            ...
            if (f6 < f7) {
                ...
                if (i3 == i || (z && rectF2.left >= rectF.left)) {
                    ...
                    fArr[1] = fDp;
                    fArr[0] = fDp;
                    float f8 = 0.7f * fDp * this.timestampsAppearing;
                    fArr[5] = f8;
                    fArr[4] = f8;
                    fArr[3] = f8;
                    fArr[2] = f8;
                    ...
                    f = fDp3;
                    float f9 = 0.7f * fDp * this.timestampsAppearing;
                    fArr2[7] = f9;
                    fArr2[6] = f9;
                    fArr2[1] = f9;
                    fArr2[0] = f9;
                    ...
                    f2 = f4;
                    float f10 = 0.7f * fDp * this.timestampsAppearing;
                    fArr3[5] = f10;
                    fArr3[4] = f10;
                    fArr3[3] = f10;
                    fArr3[2] = f10;
    ...
    private void drawTimestampLabel(Canvas canvas) {
        ...
        float f = this.selectorWidth / 2.0f;
        float fAbs = Math.abs(f - ((getMeasuredWidth() - (this.selectorWidth / 2.0f)) - (this.lastDuration > 600000 ? AndroidUtilities.dp(36.0f) : 0))) - AndroidUtilities.dp(66.0f);
        float f2 = this.lastWidth;
        if (f2 > 0.0f && Math.abs(f2 - fAbs) > 0.01f) {
            StaticLayout[] staticLayoutArr = this.timestampLabel;
            if (staticLayoutArr[0] != null) {
                staticLayoutArr[0] = makeStaticLayout(staticLayoutArr[0].getText(), (int) fAbs);
            }
            StaticLayout[] staticLayoutArr2 = this.timestampLabel;
            if (staticLayoutArr2[1] != null) {
                staticLayoutArr2[1] = makeStaticLayout(staticLayoutArr2[1].getText(), (int) fAbs);
            }
        }
        this.lastWidth = fAbs;
