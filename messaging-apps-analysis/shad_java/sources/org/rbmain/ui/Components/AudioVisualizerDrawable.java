...
public class AudioVisualizerDrawable {
        ...
        if (LiteMode.isEnabled(32)) {
            ...
            if (z3) {
                for (int i4 = 0; i4 < 6; i4++) {
                    fArr[i4] = (this.random.nextInt() % HttpStatus.HTTP_INTERNAL_SERVER_ERROR) / 1000.0f;
                }
            }
    ...
    public void draw(Canvas canvas, float f, float f2) {
        if (LiteMode.isEnabled(32)) {
            ...
            float fDp = AndroidUtilities.dp(22.0f) + (AndroidUtilities.dp(4.0f) * f8) + (this.IDLE_RADIUS * f7);
