...
public class ShareDialogCell extends FrameLayout {
    ...
    public ShareDialogCell(Context context, int i) {
        ...
        this.checkBox.setProgressDelegate(new CheckBoxBase.ProgressDelegate() { // from class: org.rbmain.ui.Cells.ShareDialogCell$$ExternalSyntheticLambda0
    ...
    protected boolean drawChild(Canvas canvas, View view, long j) {
        ...
        if (view == this.imageView && this.currentType != 2 && (tLRPC$User = this.user) != null && !MessagesController.isSupportUser(tLRPC$User)) {
            ...
            boolean z = (tLRPC$User2.self || tLRPC$User2.bot || (((tLRPC$UserStatus = tLRPC$User2.status) == null || tLRPC$UserStatus.expires <= ConnectionsManager.getInstance(this.currentAccount).getCurrentTime()) && !MessagesController.getInstance(this.currentAccount).onlinePrivacy.containsKey(Integer.valueOf(this.user.id)))) ? false : true;
            if (z || this.onlineProgress != 0.0f) {
                ...
                Theme.dialogs_onlineCirclePaint.setColor(Theme.getColor(this.currentType == 1 ? Theme.key_voipgroup_inviteMembersBackground : Theme.key_windowBackgroundWhite));
                float f = right;
                float f2 = bottom;
