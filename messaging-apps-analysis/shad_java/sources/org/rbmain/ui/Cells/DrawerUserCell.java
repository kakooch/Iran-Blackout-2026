...
public class DrawerUserCell extends FrameLayout {
    ...
    protected void onDraw(Canvas canvas) {
        int allUnreadCountAccount;
        if (UserConfig.getActivatedAccountsCount() <= 1 || !NotificationsController.getInstance(this.accountNumber).showBadgeNumber || (allUnreadCountAccount = ServiceLocator.getInstance(this.accountNumber).getCoreMainClass().getAllUnreadCountAccount()) <= 0) {
            return;
        }
        String str = String.format("%d", Integer.valueOf(allUnreadCountAccount));
