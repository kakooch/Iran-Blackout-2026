...
final class ClientCallImpl<ReqT, RespT> extends ClientCall<ReqT, RespT> {
    ...
    private void applyMethodConfig() {
        ...
        if (l != null) {
            Deadline deadlineAfter = Deadline.after(l.longValue(), TimeUnit.NANOSECONDS);
    ...
    private class DeadlineTimer implements Runnable {
        ...
        public void run() {
            ...
            long jAbs = Math.abs(this.remainingNanos);
            TimeUnit timeUnit = TimeUnit.SECONDS;
            long nanos = jAbs / timeUnit.toNanos(1L);
            long jAbs2 = Math.abs(this.remainingNanos) % timeUnit.toNanos(1L);
    ...
    private ScheduledFuture<?> startDeadlineTimer(Deadline deadline) {
        TimeUnit timeUnit = TimeUnit.NANOSECONDS;
        long jTimeRemaining = deadline.timeRemaining(timeUnit);
        return this.deadlineCancellationExecutor.schedule(new LogExceptionRunnable(new DeadlineTimer(jTimeRemaining)), jTimeRemaining, timeUnit);
    }
